  ...
  scaffor doc service create`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 && !showAll {
				...
				fmt.Printf("\nRun 'scaffor doc %s <command>' or 'scaffor doc %s --all' for details.\n", tmpl.Name, tmpl.Name)
				return nil
			}
...
Checks include:
  ...
  scaffor lint service

  # Lint a template in a custom directory
  scaffor lint -d my-templates service
  ...
  scaffor lint --all`,
	...
	cmd.Flags().StringVarP(&templateDir, "dir", "d", "", "Directory containing templates (default: .scaffor-templates)")
